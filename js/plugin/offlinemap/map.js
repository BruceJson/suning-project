...
(function() {
	...
	function La(a, b) {
		if (a = x.$(a)) {
			...
			c.K.ue && (this.cY(c.K.ue.controls), this.YL(c.K.ue.geotableId));
			...
			if (-1 < navigator.userAgent.toLowerCase().indexOf("UCBrowser") || d && e) c.K.RF = 99;
			c.Va = a;
			c.vB(a);
			a.unselectable = "on";
			a.innerHTML = "";
			...
			for (e = 0; e < z.Ar.length; e++) z.Ar[e](c);
			c.R.ZE = e;
			c.ca();
			I.load("map", function() {
				c.yb()
	...
	x.extend(La.prototype, {
		...
		hk: function(a, b, c, d) {
			...
			if (c = c && d ? f.Im(c, this.Wb) : this.mc) if (this.mc = new H(c.lng + a * e, c.lat - b * e), (a = f.oh(this.mc, this.Wb)) && g) this.gf = a
		},
		Dg: function(a, b) {
	...
	*/
	...
	function Ib(a) {
		...
		do c = Hb.indexOf(a.charAt(i++)), d = Hb.indexOf(a.charAt(i++)), f = Hb.indexOf(a.charAt(i++)), g = Hb.indexOf(a.charAt(i++)), c = c << 2 | d >> 4, d = (d & 15) << 4 | f >> 2, e = (f & 3) << 6 | g, b += String.fromCharCode(c), 64 != f && (b += String.fromCharCode(d)), 64 != g && (b += String.fromCharCode(e));
		while (i < a.length);
		return b
	}
	var O = x.lang.Xy;
		...
		function(a, b, c) {
			...
			if (t && t.yh && k() < t.yh) {
				var A = ["Moz", "O", "ms", "Webkit"],
					...
					se = function() {
						return "Promise" in a && "cast" in a.gq && "resolve" in a.gq && "reject" in a.gq && "all" in a.gq && "race" in a.gq &&
						function() {
							var b;
							new a.gq(function(a) {
								b = a
	...
	/*guoguo 这是加载模块的名字,如map_o3b5bq http://api0.map.bdimg.com/getmodules?v=2.0&mod=模块1,模块2 */
	var Sb = {
		map: "o3b5bq",  //两部分合在一起作为模块的名字，如map_o3b5bq
		common: "1jsk53",
		style: "hfbhis",
		tile: "5irhqt",
	...
	x.object.extend(I, {
		...
		load: function(a, b, c) {
			var d = this.ib(a);
			if (d.Bd == this.Fj.Yp) c && b();
			else {
				if (d.Bd == this.Fj.nG) {
					this.jK(a);
					this.wN(a);
					var e = this;
					e.BC == q && (e.BC = o, setTimeout(function() {
						for (var a = [], b = 0, c = e.Pd.Cn.length; b < c; b++) {
							var d = e.Pd.Cn[b],
								l = "";
							ja.Iy.dK(d) ? l = ja.Iy.get(d) : (l = "", a.push(d + "_" + Sb[d]));
							e.Pd.Qv.push({
								QM: d,
								IE: l
							})
						...
						/*guoguo 加载本地模块文件，在 modules 目录下 */
                        if( a.length > 0 ){
                          for(i=0; i<a.length;i++){
                            var md = offmapcfg.home + 'modules/' + a[i] + '.js';
                            pa( md );
                            my_log('加载模块:'+md);
                          }
                        } else {
		...
		jK: function(a) {
			if (a && this.oL()[a]) for (var a = this.oL()[a], b = 0; b < a.length; b++) this.jK(a[b]), this.Pd.rl[a[b]] || this.wN(a[b])
		},
		wN: function(a) {
			for (var b = 0; b < this.Pd.Cn.length; b++) if (this.Pd.Cn[b] == a) return;
			this.Pd.Cn.push(a)
		},
		xZ: function(a, b) {
			var c = this.ib(a);
		...
		nw: function(a) {
			if (a && Xa(a.id) && !isNaN(a.id)) {
				var b = {
					...
					c;
				for (c in a) b[c] = a[c];
				if (a = this.nm(a.id)) for (var d in b) a[d] = b[d];
				else this.dc.push(b)
			}
		},
		nm: function(a) {
	...
	x.extend(R, {
		...
		Ab: function(a) {
			...
			for (var d = 0; d < this.rG.length; d++) if (b.lat >= this.rG[d]) {
				c = this.pP[d];
				break
			}
			a = this.oK(a, c);
		...
		zb: function(a) {
			...
			a.lat = this.QD(a.lat, -74, 74);
			b = new H(a.lng, a.lat);
			for (var d = 0; d < this.Bu.length; d++) if (b.lat >= this.Bu[d]) {
				c = this.oG[d];
				break
			}
			if (!c) for (d = 0; d < this.Bu.length; d++) if (b.lat <= -this.Bu[d]) {
				c = this.oG[d];
				break
			}
			a = this.oK(a, c);
	...
	z.Ue(function(a) {
		...
		a.addEventListener("addoverlay", function(a) {
			...
			if (a instanceof fb) b.wa[a.ba] || (b.wa[a.ba] = a);
				...
				for (var d = q, e = 0, f = b.Be.length; e < f; e++) if (b.Be[e] === a) {
					d = o;
					break
				}
				d || b.Be.push(a)
		...
		a.addEventListener("removeoverlay", function(a) {
			...
			else for (var d = 0, e = b.Be.length; d < e; d++) if (b.Be[d] === a) {
				b.Be.splice(d, 1);
				break
			}
		});
	...
	x.extend(Gc.prototype, {
		...
		yf: function() {
			this.qh--;
			var a = this;
			this.lE && (this.map.dispatchEvent(new O("onfirsttileloaded")), this.lE = q);
		...
		wm: function(a, b, c) {
			...
			l = 0;
			for (d = a.childNodes.length; l < d; l++) w = a.childNodes[l], w.er = q, this.Me.push(w);
			if (l = this.Pm) for (var A in l) delete l[A];
			else this.Pm = {};
			...
			l = 0;
			for (d = this.Me.length; l < d; l++) n = this.Me[l], n.er || this.Ne.push(n);
			this.kn = [];
		...
		mI: function(a) {
			...
			for (var g = 0; g < c; g++) {
				...
				if (!i.$H && !(i.mp && !this.map.Vb() || i.jM && this.map.Vb())) {
					...
					w = [];
					for (d.JB = []; n < v; n++) for (y = l; y < e; y++) w.push([n, y]), d.JB.push({
						x: n,
						y: y
					});
		...
		Wg: function(a) {
			var b = this,
				c = a.target,
				a = b.map.Vb();
			if (c instanceof cb) a && !c.Em && (c.ra(this.map, this.ld), c.Em = o);
			else if (c.Ch && this.map.Wg(c.Ch), c.mp) {
		...
		xh: function(a) {
			...
			if (a instanceof cb) b && a.Em && (a.remove(), a.Em = q);
				...
				if (a.mp) for (var b = 0, c = this.Cg.length; b < c; b++) a == this.Cg[b] && this.Cg.splice(b, 1);
				else {
					b = 0;
		...
		Qt: function(a) {
			var b = this.map.pa();
			if (!this.map.Vb() && (a ? this.map.K.s_ = a : a = this.map.K.s_, a)) for (var c = p, c = "2" == z.Ny ? [z.url.proto + z.url.domain.main_domain_cdn.other[0] + "/"] : [z.url.proto + z.url.domain.main_domain_cdn.baidu[0] + "/", z.url.proto + z.url.domain.main_domain_cdn.baidu[1] + "/", z.url.proto + z.url.domain.main_domain_cdn.baidu[2] + "/"], d = 0, e; e = this.Jm[d]; d++) if (e.f_ == o) {
				b.k.$b = 18;
	...
	Pa.sk = function(a) {
		...
		var b = E.eC,
			c;
		for (c in b) if (-1 < a.search(c)) return b[c].jy;
		return ""
	};
	Pa.kL = function(a) {
		return {
	...
	ad.getTilesUrl = function(a, b) {
		var c = a.x,
			d = a.y;
	    var tdir = offmapcfg.tiles_hybrid.length > 0 ? offmapcfg.tiles_hybrid : offmapcfg.home + "tiles_hybrid";/*guoguo 使用本地卫星的瓦片 */
		tdir = tdir + "/" + b + "/" + c + "/" + d + offmapcfg.imgext; /*guoguo 使用本地卫星的瓦片 */
	    my_log('卫星底图:'+tdir);
	    return tdir;
	    /*说明：
        《卫星图_底图》
        说明，百度在线版中此处为卫星图的无标识底图，路线城市区划图为覆盖图层。
        但使用在线瓦片图下载的图像中已经拼和了底图和路线城市区划图。
        请注意《卫星图_混合覆盖图》
        */
		/*
	...
	bd.getTilesUrl = function(a, b) {
	    ...
	    /*说明：
        《卫星图_覆盖图》
        说明，百度在线版中此处为路线城市区划图为覆盖图层，底图是卫星图的无标识图。
        但使用在线瓦片图下载的图像中已经拼和了底图和路线城市区划图。
        因此，此处固定输出一张透明图。
        请注意《卫星图_底图》
        */
	...
	x.extend(Oa.prototype, {
		...
		tB: function() {
			if (this.C) for (var a = this.C.tx(), b = 0; b < a.length; b++)(a[b] instanceof T || a[b] instanceof uc) && a[b].point && this.wa.push(a[b])
		},
		rF: ba("C"),
		St: function(a) {
			this.Mf = a || "none"
	...
	function Na(a) {
		...
		if (a === q || !window.WebGLRenderingContext || x.platform.Fm && -1 == navigator.userAgent.indexOf("Android 5")) return Wd = q;
		var a = document.createElement("canvas"),
			b = p;
		try {
	...
	Ne.prototype = {
		...
		Ov: function(a) {
			...
			if (this.enabled) {
				...
				this.options.zoom && (Be && 1 < a.touches.length) && (d = de.abs(a.touches[0].pageX - a.touches[1].pageX), c = de.abs(a.touches[0].pageY - a.touches[1].pageY), this.G_ = de.sqrt(d * d + c * c), this.dy = de.abs(a.touches[0].pageX + a.touches[1].pageX - 2 * this.TF) / 2 - this.x, this.ey = de.abs(a.touches[0].pageY + a.touches[1].pageY - 2 * this.UF) / 2 - this.y, this.options.vp && this.options.vp.call(this, a));
				if (this.options.Vx && (this.options.Tk ? (c = getComputedStyle(this.Ob, p)[ne].replace(/[^0-9\-.,]/g, "").split(","), d = +(c[12] || c[4]), c = +(c[13] || c[5])) : (d = +getComputedStyle(this.Ob, p).left.replace(/[^0-9-]/g, ""), c = +getComputedStyle(this.Ob, p).top.replace(/[^0-9-]/g, "")), d != this.x || c != this.y)) this.options.Ai ? this.Wd(Je) : Le(this.TB), this.Aj = [], this.wr(d, c), this.options.ay && this.options.ay.call(this);
				this.hw = this.x;
				this.jw = this.y;
				this.du = this.x;
				this.eu = this.y;
		...
		aT: function(a) {
			...
			if (this.options.zoom && Be && 1 < a.touches.length) e = de.abs(a.touches[0].pageX - a.touches[1].pageX), f = de.abs(a.touches[0].pageY - a.touches[1].pageY), this.F_ = de.sqrt(e * e + f * f), this.Wk = o, b = 1 / this.G_ * this.F_ * this.scale, b < this.options.Vk ? b = 0.5 * this.options.Vk * Math.pow(2, b / this.options.Vk) : b > this.options.Wp && (b = 2 * this.options.Wp * Math.pow(0.5, this.options.Wp / b)), this.op = b / this.scale, e = this.dy - this.dy * this.op + this.x, f = this.ey - this.ey * this.op + this.y, this.Ob.style[ne] = "translate(" + e + "px," + f + "px) scale(" + b + ")" + Me, this.options.lN && this.options.lN.call(this, a);
			else {
				this.th = b.pageX;
				this.uh = b.pageY;
				if (0 < e || e < this.fe) e = this.options.zo ? this.x + c / 2 : 0 <= e || 0 <= this.fe ? 0 : this.fe;
				if (f > this.rf || f < this.nd) f = this.options.zo ? this.y + d / 2 : f >= this.rf || 0 <= this.nd ? this.rf : this.nd;
				this.LC += c;
				this.MC += d;
		...
		$u: function(a) {
			if (!(Be && 0 !== a.touches.length)) {
				...
				if (b.Wk) d = b.scale * b.op, d = Math.max(b.options.Vk, d), d = Math.min(b.options.Wp, d), b.op = d / b.scale, b.scale = d, b.x = b.dy - b.dy * b.op + b.x, b.y = b.ey - b.ey * b.op + b.y, b.Ob.style[ue] = "200ms", b.Ob.style[ne] = "translate(" + b.x + "px," + b.y + "px) scale(" + b.scale + ")" + Me, b.Wk = q, b.refresh(), b.options.up && b.options.up.call(b, a);
				else {
					if (b.ph) {
						...
						if (Be) if (b.EK && b.options.zoom) clearTimeout(b.EK), b.EK = p, b.options.vp && b.options.vp.call(b, a), b.zoom(b.th, b.uh, 1 == b.scale ? b.options.kW : 1), b.options.up && setTimeout(function() {
		...
		zB: function() {
			var a = this,
				...
				d = Date.now(),
				e, f, g;
			a.Vl || (a.Aj.length ? (e = a.Aj.shift(), e.x == b && e.y == c && (e.time = 0), a.Vl = o, a.ph = o, a.options.Ai) ? (a.oJ(e.time), a.wr(e.x, e.y), a.Vl = q, e.time ? a.ca(Je) : a.fo(0)) : (g = function() {
				var i = Date.now(),
					k;
				if (i >= d + e.time) {
					a.wr(e.x, e.y);
		...
		scrollTo: function(a, b, c, d) {
			...
			a = 0;
			for (b = e.length; a < b; a++) e[a].lZ && (e[a].x = this.x - e[a].x, e[a].y = this.y - e[a].y), this.Aj.push({
				x: e[a].x,
				y: e[a].y,
				time: e[a].time || 0
			});
	...
	x.extend(Oe.prototype, {
		...
		ny: function(a, b) {
			if (b || !this.$j[this.Xc] || !(this.$j[this.Xc].panoId == a && 3 !== this.$j[this.Xc].recoType)) {
				...
				!b && -1 !== d && this.$j[d] && 3 !== this.$j[d].recoType ? this.Jp(d) : this.xX(function(a) {
					for (var b = {}, d, i, k = q, l = [], m = 0, n = a.length; m < n; m++) d = a[m].catlog, i = a[m].floor, j !== d && ("" === d && j !== i ? (k = o, b[i] || (b[i] = []), b[i].push(a[m])) : (b[Qe[d]] || (b[Qe[d]] = []), b[Qe[d]].push(a[m])));
		...
		OV: function() {
			if (this.Ki) a = this.jL(this.Ni), this.qQ.innerHTML = a;
			else {
		...
		Dl: function(a) {
			this.$j = a;
			this.k.showCatalog && (0 < this.Ni.length ? (Va() ? this.OV() : this.RV(), this.Oc.offsetLeft = 60) : (this.Hs && (this.B.removeChild(this.Hs), this.Hs = p, this.Oc.fg.style.left = "0px"), this.Ki && (this.B.removeChild(this.Ki), this.Ki = p), this.Oc.offsetLeft = 0));
			var b = this.fX(a);
			Va() && (this.Ni && 0 < this.Ni.length && this.k.showExit && this.Wz) && (this.Oc.offsetLeft += this.eg.width + 8, this.Hs ? this.Hs.innerHTML = this.Wz : this.PV());
			this.Wh.innerHTML = b;
